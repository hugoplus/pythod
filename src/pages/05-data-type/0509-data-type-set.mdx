...
下面，我们来详细地学习一下 Python 中内置的集合处理方法。
...
-   `update()`：向集合中添加多个元素（可以是列表、元组或其他集合）。
...
-   `remove()`：从集合中删除单个元素。如果该元素不存在，会抛出 `KeyError` 异常。
...
-   `discard()`：从集合中删除单个元素。如果该元素不存在，不会抛出异常。
...
-   `pop()`：随机地从集合中删除一个元素，并返回该元素。如果集合为空，会抛出 `KeyError` 异常。
